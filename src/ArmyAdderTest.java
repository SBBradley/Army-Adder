...
dedicatedTransport dedicatedTransportChoice = new dedicatedTransport("Transport Truck", "3 transport trucks", "Special abilities: ignore terrain Transport capacity: 10", 500, 3, 100);
...
elites elitesChoice = new elites("Commando Squad", "5 commandos with pistols and grenades", "Special abilities: stealth, infiltrate", 1000, 5, 250);
...
fastAttack fastAttackChoice = new fastAttack("Warbikers", "10 warbikers with chains", "Special abilities: burnout, turbo", 200, 10, 100);
...
flyer flyerChoice = new flyer("Warplane Squadron", "9 warplanes with bombs and missiles", "Special abilities: strafe run, bombardment", 500, 9, 650);
...
fortification fortificationChoice = new fortification("Bunker", "1 bunker with machine gun turrets", "Special abilities: fortress, rally point", 70, 1, 700);
...
heavySupport heavySupportChoice = new heavySupport("Armored Tank", "2 armored tanks with mega cannons", "Special abilities: ignore terrain, death roller, armorclad", 300, 2, 300);
...
lordOfWar lordOfWarChoice = new lordOfWar("Berserker", "1 berserker with twin axes", "Special abilities: feel no pain, wrath", 100, 1, 200);
...
troops troopsChoice = new troops("Assualt Squad 1", "1 squad leader with rocket launcher, 9 infantrymen with rifles", "", 50, 10, 10);
troopsChoice.setTroopsTotalPointVal(150);
assertEquals("Assualt Squad 1", troopsChoice.getUnitName());
assertEquals("1 squad leader with rocket launcher, 9 infantrymen with rifles", troopsChoice.getUnitComposition());
